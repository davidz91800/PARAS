...
function calculate() {
    ...
    if (hasVisualRefs === false) {
        if (l < (H + 100)) {
            displayAlert(`<strong>ALERTE LARGEUR DZ :</strong> Sans réf. visuelles, la largeur (l) doit être ≥ à la Hauteur + 100m. (Actuel: l=${l.toFixed(0)}m, Requis: ≥${(H + 100).toFixed(0)}m)`, 'alert-error', dom.globalAlertsContainer, alertTypes);
        }
        if (l < 400) {
             displayAlert(`<strong>ALERTE LARGEUR DZ :</strong> Sans réf. visuelles, la largeur (l) doit être ≥ 400m. (Actuel: l=${l.toFixed(0)}m)`, 'alert-error', dom.globalAlertsContainer, alertTypes);
        }
    } else {
        if (l < H) {
            displayAlert(`<strong>ALERTE LARGEUR DZ :</strong> Avec réf. visuelles, la largeur (l) doit être ≥ à la Hauteur. (Actuel: l=${l.toFixed(0)}m, Requis: ≥${H.toFixed(0)}m)`, 'alert-error', dom.globalAlertsContainer, alertTypes);
        }
        if (l < 300) {
            displayAlert(`<strong>ALERTE LARGEUR DZ :</strong> Avec réf. visuelles, la largeur (l) doit être ≥ 300m. (Actuel: l=${l.toFixed(0)}m)`, 'alert-error', dom.globalAlertsContainer, alertTypes);
        }
    }
    ...
    if (windLimitResult && windLimitResult.limit !== null) {
        ...
        if (currentGroundWindMs > windLimitResult.limit) {
            displayAlert(`<strong>ALERTE VENT:</strong> Le vent au sol (${groundWindSpeed.toFixed(0)} kt) dépasse la limite réglementaire de ${windLimitResult.limit} m/s (${limitInKnots} kt) pour ces conditions.`, 'alert-error', dom.globalAlertsContainer, alertTypes);
        }
    } else if (windLimitResult) {
        displayAlert(windLimitResult.reason, 'alert-error', dom.globalAlertsContainer, alertTypes);
    }
    ...
    try {
        ...
        if (params) {
            ...
            dom.parachuteParamsContent.innerHTML = `
                <p><strong>ROF</strong> = ${rof.toFixed(1)}</p>
                <p><strong>VD</strong> = ${vd.toFixed(1)} ft/s / <strong>TFC</strong> = ${fallConst.toFixed(1)} s</p>
                <p><strong>ET</strong> = ${et.toFixed(1)} s / <strong>DQ</strong> = ${dq.toFixed(1)} s</p>
                <p><strong>FTT</strong> = ${ftt.toFixed(1)} s</p>
            `;
    ...
    if (groundSpeedKt <= 0) {
        displayAlert("Vitesse sol négative.", 'alert-error', dom.globalAlertsContainer, alertTypes);
        resetAll();
        return;
    }
    ...
    if (usableLength <= 0) {
        displayAlert("Longueur utile (L' = L - nH) est négative.", 'alert-error', dom.globalAlertsContainer, alertTypes);
        resetAll();
        return;
    }
    ...
    const theoreticalParas = Math.floor(N_perDoor) * usableDoors;
    ...
    validationMessages.forEach(msg => displayAlert(msg.text, msg.type, dom.globalAlertsContainer, alertTypes));
    if (maxParasPerPass < theoreticalParas && maxCapacity !== Infinity) {
        displayAlert(`Capacité limitée par la soute (${maxCapacity} paras).`, 'alert-info', dom.globalAlertsContainer, alertTypes);
    }
    ...
    if (dom.globalAlertsContainer.childElementCount === 0) {
        displayAlert('Tous les paramètres sont valides.', 'alert-success', dom.globalAlertsContainer, alertTypes);
    }
    
    // Update synthesis icon color based on alert types
    dom.synthesisIcon.classList.remove('status-ok', 'status-warning', 'status-error');
    if (alertTypes.has('alert-error')) {
        dom.synthesisIcon.classList.add('status-error');
    } else if (alertTypes.has('alert-warning')) {
...
function validateInputs({ H, securityLevel, parachuteType, longWind, crossWind, hasVisualRefs }) {
    ...
    if (parachuteType === 'EPC' && H < 300) {
        messages.push({ text: `Hauteur < 300m avec EPC : Le largage n'est possible que par une seule porte. La capacité/passe a été ajustée.`, type: 'alert-warning' });
    }
    const hLimits = heightLimits[parachuteType]?.[securityLevel];
    if (hLimits) {
        if (H < hLimits.min) messages.push({ text: `ALERTE HAUTEUR: ${H}m < min ${hLimits.min}m.`, type: 'alert-error' });
        if (H > hLimits.max) messages.push({ text: `ALERTE HAUTEUR: ${H}m > max ${hLimits.max}m.`, type: 'alert-error' });
    }
...
function displayAlert(message, type, targetElement, alertTypesSet) {
    alertTypesSet.add(type);
    const alertDiv = document.createElement('div');
    alertDiv.className = `alert ${type}`;
    let icon;
    if (type === 'alert-info') {
        icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 1 1-16 0 8 8 0 0 1 16 0Zm-7-4a1 1 0 1 1-2 0 1 1 0 0 1 2 0ZM9 9a.75.75 0 0 0 0 1.5h.253a.25.25 0 0 1 .244.304l-.459 2.066A1.75 1.75 0 0 0 10.747 15h.506a.75.75 0 0 0 0-1.5h-.506a.25.25 0 0 1-.244-.304l.459-2.066A1.75 1.75 0 0 0 9.253 9H9Z" clip-rule="evenodd" /></svg>`;
    } else if (type === 'alert-success') {
        icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" /></svg>`;
    } else {
